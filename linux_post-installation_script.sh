...
if [[ "${REPLY}" =~ ^[Yy]$ ]]; then
	...
	## Fedora workstation
		...
		sudo dnf install -y curl cabextract fontconfig xorg-x11-font-utils
	...
	if [[ "${REPLY}" =~ ^[Yy]$ ]]; then
	flatpak remote-add --if-not-exists flathub https://dl.flathub.org/repo/flathub.flatpakrepo
	flatpak install flathub -y org.onlyoffice.desktopeditors com.spotify.Client org.kde.kdenlive org.telegram.desktop org.shotcut.Shotcut com.valvesoftware.Steam org.videolan.VLC org.libreoffice.LibreOffice org.localsend.localsend_app org.jellyfin.JellyfinDesktop io.github.mhogomchungu.media-downloader us.zoom.Zoom #com.bitwarden.desktop org.mozilla.firefox org.mozilla.Thunderbird com.brave.Browser net.codelogistics.webapps com.vivaldi.Vivaldi
